...
export function Sidebar({ 
      ...
      {items.map((item) => {
        ...
        return (
          <Button
            ...
            variant="ghost"
            className={cn(
              "w-full justify-start gap-3 h-10 px-3 rounded-none transition-all duration-200 relative",
              isActive && "bg-primary-50 text-primary-600 border-l-2 border-primary-500",
              !isActive && "hover:bg-gray-50 text-gray-700"
            )}
            onClick={() => onViewChange(item.id)}
          >
            <div className={cn(
              "h-6 w-6 rounded flex items-center justify-center transition-colors flex-shrink-0",
              isActive 
                ? "bg-primary-100" 
                : "bg-neutral-100"
            )}>
  ...
  return (
    <aside 
      className={cn(
        "bg-background-app border-r border-neutral-200 transition-all duration-300 flex flex-col shadow-sm",
    ...
    >
      ...
      <div className="p-4 sm:p-6 border-b border-gray-200">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center shadow-sm flex-shrink-0 bg-primary-500 rounded-lg">
            <Sparkles className="h-5 w-5 text-white" />
          </div>
          <div className="min-w-0">
            <h2 className="font-bold text-neutral-800 text-sm sm:text-base truncate">LeadGen Pro</h2>
            <p className="text-xs text-neutral-500 hidden sm:block">AI-Powered Platform</p>
          </div>
        </div>
      </div>
